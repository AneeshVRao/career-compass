...
- `src/lib/auth-server.ts` — client-reachable on purpose (the root route imports `fetchAuthUser`), so it holds **no** static server import; it `await import(...)`s the request client *inside* the handler.
