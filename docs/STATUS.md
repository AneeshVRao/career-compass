...
3. **Deploy to Render.** `render.yaml` blueprint now exists at the repo root (defines the service, build/start commands, and the 7 required secrets as `sync: false` placeholders) and the production build/start path has been verified locally (`bun run build` succeeds, `PORT=3000 bun run start` serves `/` and `/board` with 200s). What's left is manual: connect the GitHub repo as a Blueprint in the Render dashboard and paste in the secret values — see the "Render setup" section in `docs/DEPLOYMENT.md`. Nothing is live yet.
